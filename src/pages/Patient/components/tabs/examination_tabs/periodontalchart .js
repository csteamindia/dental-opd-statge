...
const PeriodontalChart = ({patientData, callback, isform}) => {
  ...
  useEffect(() => {
    fatchDocs();
    setChart({...tChart, patient_id: patientData?.id, date: moment().format("YYYY-MM-DD"), doctor_id: patientData?.doctor})
    fetchChartData();
  }, []);
  ...
  const pdColumns = [
    { dataField: 'id', text: '#', style: { width: '20px' }, formatter: (cell, row, rowIndex) => (rowIndex + 1) },
