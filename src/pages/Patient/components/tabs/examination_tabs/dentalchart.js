...
import { DENTAL_CHART_EXAMINATION_URL, PATIENT_URL } from "helpers/url_helper"
...
const treatmentOptions = [
  ...
  },
  {
    ...
    options: [
      {
        value: "advance_surgical_procedure",
        label: "Advance Surgical Procedure",
      },
  ...
  },
  {
    ...
    options: [{ value: "teeth_whitening", label: "Teeth Whitening" }],
  },
  {
    category: "Other",
    options: [{ value: "add_treatment_type", label: "Add TreatmentType" }],
  },
]
...
const DentalChart = ({ patientData }) => {
  ...
  const fetchExaminationOptions = async () => {
    const { success, body } = await get(
      `${DENTAL_CHART_EXAMINATION_URL}/options`
    )
    if (success) {
  ...
  const fatchDocs = async () => {
    ...
    if (success) {
      ...
      const doc = body.filter(v => v.value == patientData?.doctor)
  ...
  const handleExamination = () => {
    ...
    obj?.toothInfo?.map(v => {
      const finalDataTemp = {
        ...
        date: getZoneDateTime(startDate).format("YYYY-MM-DD HH:mm:ss"),
  ...
  const fetchChartData = async () => {
    const { success, body } = await get(
      `dentalchart?patient_id=${patientData?.id}`
    )
    if (success) {
      let a = []
      body?.items?.map(v => {
    ...
    <Card>
      <CardBody>
        <Row>
          <Col md={8}>
            <Toothchart
              callback={handleToothClick}
              selectedTeeth={selectedTeeth}
              selectedTrat={selectedTrat}
            />
          </Col>
          <Col md={4}>
            <h5>Examination</h5>
            ...
            <div className="mb-2">
              <Select
                ...
                value={doctorsOptions?.filter(
                  v =>
                    v.value == formData?.doctor ||
                    v.value == patientData?.doctor
                )}
                placeholder="Select doctor"
                onChange={selectedOption => {
                  setFormData(p => ({
                    ...p,
                    doctorOptions: selectedOption,
            ...
            <table className="table">
              ...
              <tbody>
                {finalData?.map((item, index) => (
                  <tr key={`DCT_${index + 1}`}>
                    <td>{getZoneDateTime(item?.date).format("DD-MM-YYYY")}</td>
