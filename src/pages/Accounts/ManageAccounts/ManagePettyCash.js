...
const ManagePettyCash = () => {
  ...
  const [voucherData, setVoucherData] = useState({
    voucherDate: getZoneDateTime().format("YYYY-MM-DD HH:mm"),
  ...
  const manageColumns = [
    ...
    },
    {
      ...
      text: "Description",
      formatter: (cell, row) =>
        row.transection_description.replaceAll("_", " "),
    },
    {
      ...
      formatter: function ActionButtons(cell, row) {
        return (
          <>
            <a
              href="#"
              className="btn btn-info btn-sm edit"
  ...
  const handleSubmit = async () => {
    const ent = [...voucherData.voucherEntries, voucherData.currentEntry]?.map(
      entry => ({
  ...
  const handleTotalValueCalc = () => {
    ...
    if (voucherDataEntries.length > 0) {
      let totalAmount = voucherDataEntries.reduce(
  ...
  const Reciept = [{ value: "Cash_Recived", label: "Cash Recived" }]
  ...
  const voucherOptions =
    voucherData?.voucherType[0]?.value == 1 ? Reciept : Payment
              ...
              <Row key={index} className="mb-2">
                ...
                <Col>
                  <input
                    ...
                    placeholder="Amount"
                    onChange={e => handleEntryUpdate(index, "amount", e)}
                  />
                </Col>
                <Col>
                  <input
                    ...
                    placeholder={`Paid ${
                      voucherData?.voucherType[0]?.value === "1" ? "By" : "To"
                    }`}
                    onChange={e => handleEntryUpdate(index, "remark", e)}
                  />
                </Col>
                <Col md={1}>
                  <button
