...
import { TAGS_URL } from 'helpers/url_helper';
...
const ManageTags = () => {
    ...
    const fetchData = async () => {
        const { success, body } = await get(`${TAGS_URL}?page=${pagination.page}&limit=${pagination.limit}&search=${pagination.Search}`);
        if (success) {
            setLoading(true);
            setRows(body?.body);
            setLoading(false);
    ...
    const handleSubmit = async () => {
        ...
        const url = editMode ? `${TAGS_URL}/${editingId}` : TAGS_URL;
        const method = editMode ? put : post;

        const { success, body } = await method(url, formData);

        if (success) {
            showSuccessAlert(editMode ? 'Tag updated successfully!' : 'Tag created successfully!');
            setIsForm(false);
            setFormData({});
            setEditMode(false);
            setEditingId(null);
    ...
    const handleDelete = async (id, sts) => {
        ...
        const { isConfirmed } = await showConfirmAlert(`${action} this Tag`, `Do you really want to ${action} this Tag?`,
            `Yes, ${action} it!`, `${action}d!`, `Tag has been ${action}d successfully.`);

        if (isConfirmed) {
            const { success } = await del(`${TAGS_URL}/${id}?sts=${sts}`);
            if (success) {
                fetchData();
            }
        }
                            ...
                            <CardBody>
                                <Row>
                                    <div className="d-flex justify-content-between mb-2">
                                        <h5> <i role="button" className="me-2 fas fa-arrow-left" onClick={handleToggle} /> Create Tag </h5>
                                        <button className="btn btn-danger" onClick={handleToggle}> <i className="mdi mdi-close noti-icon" /> Cancel </button>
                                    </div>
                                    <hr />
                                    <Col md={4}>
                                        <div className="mb-2">
                                            <AvField name="title" label="Tag / Title" type="text" placeholder="Enter Tag" value={formData.title} onChange={handleChange} required errorMessage="Please enter Tag" />
                                        </div>
                                    </Col>
                                </Row>
