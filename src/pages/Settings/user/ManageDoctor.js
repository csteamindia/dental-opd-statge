...
import { CHAIR_URL, DOCTOR_URL, DOCTOR_TIMING_URL } from "helpers/url_helper"
...
const ManageDoctor = () => {
  ...
  const [avalableChairs, setAvalableChairs] = useState([])
  ...
  const fetchData = async () => {
    ...
    const { success, body } = await get(
      `${DOCTOR_URL}?page=${pagination.page}&limit=${pagination.limit}&search=${pagination.Search}`
    )
    if (success) {
      setRows(body)
      setLoading(false)
  ...
  const handleDelete = async (id, sts) => {
    ...
    const { isConfirmed } = await showConfirmAlert(
      `${action} Bank`,
      `Do you really want to ${action} this Bank?`,
      `Yes, ${action} it!`,
      `${action}d!`,
      `Bank has been ${action}d successfully.`
    )
  ...
  useEffect(() => {
    ...
    const fetchChairs = async () => {
      const { success, body } = await get(`${CHAIR_URL}?page=1&limit=100`)
      if (success) {
        const available = body.items
          .filter(chair => chair.status === 0)
          .map(chair => ({
            value: chair.id,
            label: chair.title,
          }))
        setAvalableChairs(available)
      }
    }
    fetchChairs()
  }, [isForm])
            ...
            <Card>
              <CardBody>
                  ...
                  <Col md={5}>
                    <div className="mb-2">
                      <AvField
                        ...
                        type="text"
                        placeholder="Enter Licence / Registration Number"
                        value={formData.registration_no}
                        disabled={editMode}
                        onChange={handleChange}
                        required
                        errorMessage="Please enter Licence / Registration Number"
                      />
                    </div>
                  </Col>
                </Row>
                <Row className="mt-2">
                  ...
                  <Col>
                    <div className="mb-2">
                      <label>Doctor Chair</label>
                      <Select
                        isClearable
                        options={avalableChairs}
                        value={avalableChairs.find(
                          v => v.value == formData.doc_chair
                        )}
                        onChange={val =>
                          setFormData(prev => ({
                            ...prev,
