...
const { Doctors, Chairs, Clinics, User } = db
...
const getAllChairsService = async req => {
  try {
    const {
      query: { clinic_id, client_id, page = 1, limit = 10 },
      body,
      tokendata,
    } = req
    ...
    if (tokendata.clinic_id != null) {
      const getChairId = await Doctors.findOne({
        attributes: ["doc_chair"],
        where: {
          code: tokendata.doc_code,
        },
      })
      const id = getChairId.doc_chair
      where.id = id
    }
    ...
    if (clinic_id) {
      condition.where.clinic_id = clinic_id
    }
