...
const updateWPTService = async (id, updateData) => {
  try {
    ...
    if (updatedCount === 0) {
      return {
        success: false,
        message: "No record updated. WPT may not exist.",
      }
    }
...
const updateClinicConfigService = async (req, id) => {
  try {
    const { wp_key, wp_sessionid } = req.body
    const [updatedCount] = await Clinics.update(
      { wp_key, wp_sessionid },
      { where: { id } }
    )
...
const getClinicConfigService = async id => {
  try {
    const res = await Clinics.findOne({
      attributes: ["wp_key", "wp_sessionid"],
      where: { id },
    })
...
export {
  ...
  updateWPTService,
  getWPTService,
  updateClinicConfigService,
  getClinicConfigService,
}
