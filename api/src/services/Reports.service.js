...
const appointmentWaitningReportsService = async data => {
  const { clinic_id, dates, doctor_code, daterange, limit, page, search } =
    data.query
  const parsedPage = parseInt(page) || 1
  const parsedLimit = limit === "all" ? null : parseInt(limit) || 10
  const offset = parsedLimit ? (parsedPage - 1) * parsedLimit : null
  ...
  try {
    ...
    if (res.count !== 0) {
      res.rows = JSON.parse(JSON.stringify(res.rows)).map(row => {
        ...
        const start = moment(arrTime).tz(req.timezone)
        const end = attTime
          ? moment(attTime).tz(req.timezone)
          : moment().tz(req.timezone) // Use current time if null
    ...
    if (res.count !== 0) {
      ...
      res.rows = await Promise.all(
        parsedRows.map(async row => {
          // Parse JSON field
          const CG = row.communication_group
            ? JSON.parse(row.communication_group)
            : []
...
const doctorsWorkReportsService = async data => {
  const { clinic_id, date, doctor_code, daterange, limit, page, search } =
    data.query
  const parsedPage = parseInt(page) || 1
  const parsedLimit = limit === "all" ? null : parseInt(limit) || 10
  const offset = parsedLimit ? (parsedPage - 1) * parsedLimit : null
  ...
  if (daterange)
    where.created_at = { [Sequelize.Op.between]: daterange.split(",") }
    ...
    P: {
      ...
      attributes: [
        [
          Sequelize.fn(
            "CONCAT",
            Sequelize.col("ref_patient.title"),
            " ",
            Sequelize.col("ref_patient.first_name"),
            " ",
            Sequelize.col("ref_patient.last_name")
          ),
          "name",
        ],
      ],
  ...
  const conditions = {
    ...
    attributes: [
      [
        Sequelize.fn(
          "CONCAT",
          Sequelize.col("Patients.title"),
          " ",
          Sequelize.col("Patients.first_name"),
          " ",
          Sequelize.col("Patients.last_name")
        ),
        "full_name",
      ],
      "mobile",
...
const birthdayReportsService = async data => {
  const { clinic_id, dobdaterange, doctor, patient_id, limit, page, search } =
    data.query
  const parsedPage = parseInt(page) || 1
  const parsedLimit = limit === "all" ? null : parseInt(limit) || 10
  const offset = parsedLimit ? (parsedPage - 1) * parsedLimit : null
...
export {
  ...
  birthdayReportsService,
  appointmentReportService,
  appointmentWaitningReportsService,
  patientgReportsService,
  doctorsWorkReportsService,
  patientFilesReportsService,
  referenceReportsService,
}
