...
const createVoucherService = async req => {
  ...
  try {
    ...
    if (
      ...
      !body.voucherType ||
      !body.voucherEntries ||
      !Array.isArray(body.voucherEntries)
    ) {
      throw new Error("Missing required fields")
    }

    const t_id = await GenrateTrasectionId(
      body.voucherType == 1 ? "VCH" : "PAY"
    )
    ...
    const voucher = {
      ...
      transection_description:
        body.voucherType == 1 ? "Cash_Recived" : "Cash_Paid",
    }
...
const createBillingService = async req => {
  ...
  try {
    ...
    const finalObj = {
      ...
      date: moment().tz(req.timezone).format("YYYY-MM-DD HH:mm:ss"),
    ...
    if (billingResult) {
      const a = {
        billed_date: moment().tz(req.timezone).format("YYYY-MM-DD HH:mm:ss"),
        is_billed: 1,
      }
...
const cancelBillingService = async req => {
  try {
    const { id } = req.params
    // const { sts } = req.query;
    const res = await Billing.update({ status: 3 }, { where: { id } })
    return res
  } catch (e) {
    console.log(e)
    throw e
