...
export const generateAccessToken = data => {
  try {
    ...
    return jwt.sign(data, privateKey, { expiresIn: "1d", algorithm: "RS256" })
  } catch (error) {
...
export const generateRefreshToken = data => {
  data = JSON.parse(JSON.stringify(data))
  return jwt.sign(data, privateKey, { expiresIn: "7d", algorithm: "RS256" })
}
  ...
  try {
    ...
    if (!accessToken) {
      return res
        ...
        .send(responseHandler("Access token not found", false))
    }
    ...
    try {
      ...
      if (!refreshToken) {
        return res
          .status(httpStatus.UNAUTHORIZED)
          .send(responseHandler("Refresh token not found", false))
      }
      ...
      try {
        ...
        return res
          .status(httpStatus.UNAUTHORIZED)
          .send(responseHandler("Invalid refresh token", false))
      }
    }
  } catch (error) {
    return res
      .status(httpStatus.INTERNAL_SERVER_ERROR)
      .send(responseHandler("Authentication failed", false))
  }
}
