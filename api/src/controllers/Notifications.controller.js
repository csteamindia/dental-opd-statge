...
const getClinicConfig = async (req, res) => {
  try {
    const { id } = req.params
    const data = await getClinicConfigService(id)
    res.status(httpStatus.OK).send(responseHandler(data.data, true))
  } catch (e) {
    res
      .status(httpStatus.INTERNAL_SERVER_ERROR)
      .send(responseHandler([], false, e.message))
...
const updateClinicConfig = async (req, res) => {
  try {
    const { id } = req.params
    const data = await updateClinicConfigService(req, id)
    res.status(httpStatus.OK).send(responseHandler(data.body, true))
  } catch (e) {
    res
      .status(httpStatus.INTERNAL_SERVER_ERROR)
